 ...
 * GenericForms
 *
 * Interface.Forms.GenericForms.html page content scripts. Initialized from scripts.js file.
 *
 *
 */
...
class GenericForms {
  ...
  _initContact() {
    ...
    if (jQuery().select2) {
      jQuery('#contactUnitinduk').select2({minimumResultsForSearch: Infinity, placeholder: ''});
      jQuery('#contactUnitinduk').on('change', function () {
        jQuery(this).valid();
      });
    }

    if (jQuery().select2) {
      jQuery('#contactUnitpelaksana').select2({minimumResultsForSearch: Infinity, placeholder: ''});
      jQuery('#contactUnitpelaksana').on('change', function () {
        jQuery(this).valid();
      });
    }

    if (jQuery().select2) {
      jQuery('#contactSubunitpelaksana').select2({minimumResultsForSearch: Infinity, placeholder: ''});
      jQuery('#contactSubunitpelaksana').on('change', function () {
        jQuery(this).valid();
      });
    }
    ...
    const validateOptions = {
      rules: {
        ...
        contactUnitpelaksana: {
          required: true,
        },
        contactSubunitpelaksana: {
          required: true,
        },
        contactMessage: {
          required: true,
    ...
    form.addEventListener('submit', (event) => {
      ...
      if (jQuery(form).valid()) {
        const formValues = {
          ...
          contactUnitinduk: form.querySelector('[name="contactUnitinduk"]').value,
          contactUnitpelaksana: form.querySelector('[name="contactUnitpelaksana"]').value,
          contactSubunitpelaksana: form.querySelector('[name="contactSubunitpelaksana"]').value,
