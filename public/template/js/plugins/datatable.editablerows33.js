...
class EditableRows {
    constructor() {
      ...
      this._createInstance2();
      this._addListeners();
      this._addListeners2();
      this._extend();
      this._extend2();
      this._initBootstrapModal();
      this._initBootstrapModal2();
    }
  
    // Creating datatable instance
    _createInstance() {
      const _this = this;
      this._datatable = jQuery('#datatableRows, #datatableRows2, #datatableRows1').DataTable({
        paging: false,
        scrollX: true,
        ...
        pageLength: 5,
        columns: [{data: 'no'}, {data: 'nomor surat'}, {data: 'nip'}, {data: 'nama'}, {data: 'jenis mutasi'}, {data: 'hasil evaluasi'}, {data: 'hasil konfirmasi'}, {data: 'status'}, {data: 'action'}, {data: 'action22'} ],
        language: {
            paginate: {
    ...
    _createInstance2() {
        const _this = this;
        this._datatable = jQuery('#datatableRowsdeval').DataTable({
          paging: false,
          scrollX: true,
          buttons: ['copy', 'excel', 'csv', 'print'],
          info: false,
          ...
          sDom: '<"row"<"col-sm-12"<"table-container"t>r>><"row"<"col-12"p>>', // Hiding all other dom elements except table and pagination
          pageLength: 10,
          columns: [{data: 'no'}, {data: 'Nomor Surat/ND'}, {data: 'Dari'}, {data: 'Jml Usulan/Kebutuhan'}, {data: 'Eval PIC'}, {data: 'Eval MSB'}, {data: 'Eval VP'}, {data: 'Lanjut'}, {data: 'Tdk Lanjut'}, {data: 'Jawaban'}, {data: 'Konfirmasi'}, {data: 'Balasan Konfirmasi'}, {data: 'Cetak SK'} ],
          language: {
              paginate: {
                previous: '<i class="cs-chevron-left"></i>',
                next: '<i class="cs-chevron-right"></i>',
