...
<div class="card shadow-lg">
    <div class="card-body">
        <div class="btn-group">
            <a href="<?= site_url('masterdata/viewdataaps') ?>" class="btn-link">
                <i data-cs-icon="chevron-left" class="mt-2 me-2" data-cs-size="15"></i>
            </a>
            <h2 class="small-title mt-2">Upload Data APS</h2>
        </div>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  document.getElementById('uploadBtn').addEventListener('click', function(){
    ...
    fetch("<?= base_url('apsimport/upload') ?>", {
      method: 'POST', body: fd,
  ...
  function processNext(){
    ...
    fetch("<?= base_url('apsimport/processChunk') ?>", {
      method: 'POST', body: fd,
      headers: {'X-Requested-With':'XMLHttpRequest'}
    })
    .then(r => r.json())
