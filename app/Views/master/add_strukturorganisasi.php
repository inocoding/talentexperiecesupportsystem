...
<?= $this->section('title') ?>
<title>Upload Data Struktur Organisasi</title>
<?= $this->endSection() ?>
...
<div class="card shadow-lg">
  <div class="card-body">
    <!-- Back Button Start -->
    <a href="<?= site_url('masterdata/data_struktur_organisasi') ?>" class="btn btn-icon btn-icon-only btn-foreground-alternate shadow add-dapeg" data-bs-toggle="tooltip" data-bs-placement="top" title="Back" type="button" data-bs-delay="0">
      <i data-cs-icon="arrow-left"></i>
    </a>
    <!-- Back Button End --><br><br>
    <p>Upload Data Struktur Organisasi</p>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('uploadBtn').addEventListener('click', function() {
      ...
      fetch("<?= base_url('Strukturorganisasiimport/upload') ?>", {
          method: 'POST',
          body: fd,
          headers: {
    ...
    function processNext() {
      const fd = new FormData();
      injectCSRF(fd);
      fetch("<?= base_url('Strukturorganisasiimport/processChunk') ?>", {
          method: 'POST',
          body: fd,
          headers: {
            'X-Requested-With': 'XMLHttpRequest'
        ...
        .then(res => {
          if (res.status === 'ok') {
            ...
            if (res.done) {
              document.getElementById('status').textContent = 'Proses selesai!';
            } else {
              processNext();
            }
          } else {
