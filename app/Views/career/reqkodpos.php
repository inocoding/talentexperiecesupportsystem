...
<?= $this->section('title') ?>
<title>Request Kode Posisi</title>
<?= $this->endSection() ?>
...
<div class="page-title-container">
    <div class="row">
        <div class="btn-group">
            <h2 class="small-title mt-2">Request Kode Posisi</h2>
        </div>
    </div>
</div>
                    ...
                    <table id="datatableRowsdeval" class="data-table nowrap hover">
                        <thead>
                            <tr>
                                ...
                                <th class="text-muted text-small text-uppercase">Judul Template</th>
                                <th class="text-muted text-small text-uppercase">Jumlah Data</th>
                        ...
                        <tbody style="font-size: smaller;">
                            ...
                            foreach ($user as $key => $value) : ?>
                                <tr>
                                    <td><?= $no++ ?></td>
                                    <td><?= $value->judul_req ?></td>
                                    <td><?= $value->jml_peg_req ?></td>
                                    <td><?= $value->tgl_req ?></td>
                                    <td>
                                        <div class="d-inline-block text-center">
                                            <form action="<?= site_url('career/del_req/' . $value->id_req) ?>" method="post" class="d-inline">
                                                <?= csrf_field() ?>
                                                <a class="btn btn-icon btn-icon-only btn-foreground-alternate" href="<?= site_url('career/exportreq/' . $value->id_req) ?>" data-bs-offset="0,3">
                                                    <i data-cs-icon="download"></i>
                                                </a>
                                                <!-- <a class="btn btn-icon btn-icon-only btn-foreground-alternate disabled" href="<?= site_url('career/exportpdf/' . $value->id_req) ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="Pdf">
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Request Kode Posisi </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="<?= site_url('career/createreq') ?>" method="POST" enctype="multipart/form-data">
                <?= csrf_field() ?>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6">
                            <label for="inputAddress2" class="form-label">Judul Template</label>
                            <input name="judul_req" type="text" class="form-control" />
                        </div>
                        <div class="col-md-3">
                            <label for="inputPassword4" class="form-label">Tanggal</label>
                            <input type="date" name="tgl_req" class="form-control" />
                        </div>
                        <div class="col-md-3">
                            <label for="inputPassword4" class="form-label">Jumlah Data</label>
                            <input id="getDataa" type="number" name="jml_peg_req" class="form-control" />
                            <input id="" type="hidden" name="req_oleh" class="form-control" value="<?= userLogin()->nip ?>" />
                        </div>
                        <div class="col-md-6 mt-2">
                            <label for="inputPassword4" class="form-label">Left Signature</label>
                            <div class="input-group">
                                <select name="left_sign_req" class="form-select" id="inputGroupSelect04" aria-label="Example select with button addon">
                        ...
                        <div class="col-md-6 mt-2">
                            ...
                            <div class="input-group">
                                <select name="right_sign_req" class="form-select" id="inputGroupSelect04" aria-label="Example select with button addon">
                                    <option selected>Choose...</option>
                                    <option value="Andrisa">Andrisa</option>
                                    <option value="Diah Dayanti">Diah Dayanti</option>
                                    <option value="Gregorius Helmy Widyapamungkas">Gregorius Helmy Widyapamungkas</option>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary">Buat Request Kode Posisi</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    $('#getDataa').on('change', function() {
        $.ajax({
            url: "<?= site_url('career/get_peg_req') ?>",
            dataType: "json",
