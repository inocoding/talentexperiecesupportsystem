...
<div class="page-title-container">
    <div class="row">
        <div class="btn-group">
            <h2 class="small-title mt-2">Lampiran Evaluasi Mutasi</h2>
        </div>
    </div>
</div>
                    ...
                    <table id="datatableRowsdeval" class="data-table nowrap hover">
                        <thead>
                            <tr>
                                ...
                                <th class="text-muted text-small text-uppercase">Judul Lampiran</th>
                                <th class="text-muted text-small text-uppercase">Tanggal Lampiran</th>
                                <th class="text-muted text-small text-uppercase">Jumlah Pegawai Eval</th>
                        ...
                        <tbody style="font-size: smaller;">
                            ...
                            foreach ($user as $key => $value) : ?>
                                <tr>
                                    <td><?= $no++ ?></td>
                                    <td><?= $value->nomor_lampeval ?></td>
                                    <td><?= $value->tgl_lampeval ?></td>
                                    <td><?= $value->jml_peg_lampeval ?></td>
                                    <td>
                                        <div class="d-inline-block text-center">
                                            <form action="<?= site_url('career/del_lampeval/' . $value->id_lamp) ?>" method="post" class="d-inline">
                                                <?= csrf_field() ?>
                                                <a class="btn btn-icon btn-icon-only btn-foreground-alternate" href="<?= site_url('career/export/' . $value->id_lamp) ?>" data-bs-offset="0,3">
                                                    <i data-cs-icon="download"></i>
                                                </a>
                                                <!-- <a class="btn btn-icon btn-icon-only btn-foreground-alternate disabled" href="<?= site_url('career/exportpdf/' . $value->id_lamp) ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="Pdf">
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Lampiran Evaluasi </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="<?= site_url('career/createlampeval') ?>" method="POST" enctype="multipart/form-data">
                <?= csrf_field() ?>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6">
                            <label for="inputAddress2" class="form-label">Judul Lampiran</label>
                            <input name="nomor_lampeval" type="text" class="form-control" />
                        </div>
                        <div class="col-md-3">
                            <label for="inputPassword4" class="form-label">Tanggal Lampiran</label>
                            <input type="date" name="tgl_lampeval" class="form-control" />
                        </div>
                        <div class="col-md-3">
                            <label for="inputPassword4" class="form-label">Jumlah Pegawai</label>
                            <input id="getDataa" type="number" name="jml_peg_lampeval" class="form-control" />
                            <input id="" type="hidden" name="lamp_oleh" class="form-control" value="<?= userLogin()->nip ?>" />
                        </div>
                        <div class="col-md-6 mt-2">
                            <label for="inputPassword4" class="form-label">Left Signature</label>
                            <div class="input-group">
                                <select name="left_sign" class="form-select" id="inputGroupSelect04" aria-label="Example select with button addon">
                        ...
                        <div class="col-md-6 mt-2">
                            ...
                            <div class="input-group">
                                <select name="right_sign" class="form-select" id="inputGroupSelect04" aria-label="Example select with button addon">
                                    <option selected>Choose...</option>
                                    <option value="Andrisa">Andrisa</option>
                                    <option value="Diah Dayanti">Diah Dayanti</option>
                                    <option value="Gregorius Helmy Widyapamungkas">Gregorius Helmy Widyapamungkas</option>
                        ...
                        <div class="col-12 mt-5">
                            <label for="inputPassword4" class="form-label">Data Pegawai Eval</label>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary">Buat Lampiran Evaluasi</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    $('#getDataa').on('change', function() {
        $.ajax({
            url: "<?= site_url('career/get_peg_lampeval') ?>",
            dataType: "json",
