...
class OrgKpDuaImport extends BaseController
{
    // konfigurasi chunck
    ...
    public function processChunk(){
        ...
        for ($row = $start; $row <= $end ; $row++) { 
            ...
            $rows[] = [
                'kode_org_kp_dua'  => $val('A'),
                'nama_org_kp_dua'  => $val('B'),
                'singkatan_kp_dua' => $val('C'),
                'parent_org_kp_satu'   => $val('D'),
            ];
        }

        // insert batch
        if (!empty($rows)) {
            (new OrgKpDuaModel())->insertBatch($rows, 500);
        }
