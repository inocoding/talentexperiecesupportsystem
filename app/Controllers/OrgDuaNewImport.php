...
class OrgDuaNewImport extends BaseController
{
    // konfigurasi chunck
    ...
    public function processChunk(){
        ...
        for ($row = $start; $row <= $end ; $row++) { 
            ...
            $KodeOrgDua = trim((string) $sheet->getCell("A{$row}")->getValue());
            if ($KodeOrgDua === '') continue;
        ...
        //    }
            ...
            // normalisasi enum
            $rows[] = [
                'kode_org_dua'              => $KodeOrgDua,
                'nama_org_dua'              => $val('B'),
                'singkatan'                  => $val('C'),
                'parent_org_dua'             => $val('D'),
            ];
        }

        // insert batch
        if (!empty($rows)) {
            (new OrgDuaNewModel())->insertBatch($rows, 500);
        }
