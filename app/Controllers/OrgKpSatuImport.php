...
class OrgKpSatuImport extends BaseController
{
    // konfigurasi chunck
    ...
    public function processChunk(){
        ...
        for ($row = $start; $row <= $end ; $row++) { 
            ...
            $rows[] = [
                'kode_org_kp_satu'  => $val('A'),
                'nama_org_kp_satu'  => $val('B'),
                'singkatan_kp_satu' => $val('C'),
                'parent_org_satu'   => $val('D'),
            ];
        }

        // insert batch
        if (!empty($rows)) {
            (new OrgKpSatuModel())->insertBatch($rows, 500);
        }
